...
import { formatCurrency, STATUS_LABELS, DESIGN_TYPE_LABELS } from '@/utils/formatters';
...
export default function DesignPage() {
  ...
  const filtered = useMemo(() => orders.filter(o => {
    ...
    return matchSearch && (!statusFilter || o.status === statusFilter) && (!labelFilter || o.businessLabel === labelFilter);
    ...
    <DashboardLayout title="قسم التصميم">
      <div className="space-y-4 animate-fade-in">
        ...
        {loading ? <LoadingSpinner /> : filtered.length === 0 ? (
          <EmptyState title="لا توجد طلبات تصميم" icon={<Pen className="w-8 h-8 text-slate-600" />}
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-slate-800 bg-slate-800/40">
                    ...
                    <th className="px-4 py-3 text-slate-400 font-medium text-xs uppercase tracking-wide hidden lg:table-cell text-right">نوع التصميم</th>
                    ...
                    <th className="px-4 py-3 text-slate-400 font-medium text-xs uppercase tracking-wide hidden lg:table-cell text-right">السعر</th>
                    ...
                    <tr key={order.id} className={`hover:bg-slate-800/40 transition-colors ${isLate(order) ? 'bg-red-500/3' : ''}`}>
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2">
                          <span className={`font-medium ${isLate(order) ? 'text-red-300' : 'text-slate-200'}`}>{order.orderName}</span>
                          {isLate(order) && <LateBadge />}
                        </div>
                      </td>
                      <td className="px-4 py-3 text-slate-400 hidden md:table-cell">{order.clientName}</td>
                      <td className="px-4 py-3 text-slate-500 hidden lg:table-cell">{order.designType ? DESIGN_TYPE_LABELS[order.designType] : '—'}</td>
                      ...
                      <td className="px-4 py-3 text-slate-400 hidden lg:table-cell">{formatCurrency(order.price)} ر.س</td>
                      <td className="px-4 py-3">

...
import { formatCurrency, DESIGN_TYPE_LABELS } from '@/utils/formatters';
...
export default function DesignOrderDetailPage() {
  ...
  return (
    <DashboardLayout title="تفاصيل طلب التصميم">
      <div className="max-w-4xl space-y-4">
        <div className="flex items-center justify-between">
          <Link href="/design" className="flex items-center gap-1 text-slate-500 hover:text-slate-800 text-sm"><ArrowRight className="w-4 h-4" />العودة</Link>
          <div className="flex gap-2">
            <Link href={`/design/${id}/edit`}><Button variant="outline" size="sm" icon={<Pencil className="w-3.5 h-3.5" />}>تعديل</Button></Link>
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-sm">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-5">
            <Detail label="نوع التصميم" value={order.designType ? DESIGN_TYPE_LABELS[order.designType] : '—'} />
            ...
            <Detail label="إنشاء الطلب" value={formatDate(order.createdAt)} />
          </div>
        </div>
        <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-sm">
        ...
        {order.notes && (
          <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-sm">
            <h3 className="text-slate-900 font-semibold mb-2">ملاحظات</h3>
            <p className="text-slate-600 text-sm whitespace-pre-wrap">{order.notes}</p>
          </div>
        )}
      </div>
      <ConfirmDialog open={confirmDelete} onClose={() => setConfirmDelete(false)} onConfirm={async () => { setDeleting(true); await deleteOrder(id); router.push('/design'); }} loading={deleting} />
    </DashboardLayout>
  );
}

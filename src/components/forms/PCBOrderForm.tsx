...
export default function PCBOrderForm({ initial, onSubmit, loading }: Props) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <Input label="نوع اللوحة" value={form.boardType} onChange={e => set('boardType', e.target.value)} placeholder="مثال: FR4" />
        <Input label="حجم اللوحة" value={form.boardSize} onChange={e => set('boardSize', e.target.value)} placeholder="مثال: 100x100mm" />
        <Input label="الكمية" type="number" min="1" value={form.quantity} onChange={e => set('quantity', e.target.value)} />
        <Input label="عدد الطبقات" type="number" min="1" value={form.layers} onChange={e => set('layers', e.target.value)} />
